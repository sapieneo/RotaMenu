...
- **v3 — sabit (sticky) foto + crossfade:** Hero fotoğraf artık `position: sticky` (`top-14`, nav'ın hemen altı) + kendi yüksekliğini negatif `margin-bottom` ile akıştan sıfırlıyor. Sonuç: o kategorinin ürünleri kaydırılırken foto ekranda sabit kalıyor; ürünler bitip sıradaki kategoriye geçilince foto da değişiyor. `active` (mevcut scroll-spy state'i, sekme çubuğuyla PAYLAŞILIYOR) üzerinden `opacity` + `transition-opacity duration-700` ile iki foto arasında yumuşak geçiş sağlanıyor — global sabit katman yerine tamamen kendi `<section>`'ına özgü (scope'lu), bu yüzden diğer kategorilerde/başlık-altbilgide "beyazın arkasından resim sızması" riski yok. Üstteki kategori sekme çubuğu zaten `sticky top-0` + aktif sekmeyi yatayda `scrollTo({behavior:'smooth'})` ile ortalıyordu (önceki turda doğrulandı) — bu davranış aynı `active` state'ini kullandığı için foto geçişiyle senkron.
